...
class TcpProxyTest {
    ...
    @Test
    void testProxying() {
        assertDoesNotThrow(() -> startServerAndProxyAnd((serverSocketChannel, tcpProxy) ->
                connectViaProxyAnd(serverSocketChannel, tcpProxy, (clientChannel, serverChannel) -> {
                    sendString(clientChannel, "hello");
                    pause(100);
                    assertEquals("hello", receiveString(serverChannel), "traffic forwarded");
                })), "proxy forwards traffic");
    }

    @Test
    void testCanSuspendTraffic() {
        assertDoesNotThrow(() -> startServerAndProxyAnd((serverSocketChannel, tcpProxy) ->
                connectViaProxyAnd(serverSocketChannel, tcpProxy, (clientChannel, serverChannel) -> {
                    ...
                    sendString(clientChannel, "hello");
                    pause(100);
                    assertEquals("", receiveString(serverChannel), "traffic suspended");
                })), "proxy can suspend traffic");
    }

    @Test
    void testCanDropConnections() {
        assertDoesNotThrow(() -> startServerAndProxyAnd((serverSocketChannel, tcpProxy) ->
                connectViaProxyAnd(serverSocketChannel, tcpProxy, (clientChannel, serverChannel) -> {
    ...
    @Test
    void testCanReconnectAfterDroppingConnections() {
        assertDoesNotThrow(() -> startServerAndProxyAnd((serverSocketChannel, tcpProxy) -> {
            connectViaProxyAnd(serverSocketChannel, tcpProxy, (clientChannel, serverChannel) -> {
                tcpProxy.dropConnectionsAndPauseNewConnections();
                waitForCondition("server didn't disconnect", () -> atEndOfStream(serverChannel), TIMEOUT_MS);
            ...
            connectViaProxyAnd(serverSocketChannel, tcpProxy, (clientChannel, serverChannel) -> {
                sendString(clientChannel, "hello");
                pause(100);
                assertEquals("hello", receiveString(serverChannel), "traffic forwarded after reconnect");
            });
        }), "proxy can reconnect after dropping connections");
    }
